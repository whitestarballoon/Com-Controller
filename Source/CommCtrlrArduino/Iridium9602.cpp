...
void Iridium9602::initModem()
{
   ...
   { 
	...
	if (HIGH == digitalRead(pinDSR)) 
	{
	 ...
	 //NetworkAvailableJustChanged = false;
	 //SatelliteNetworkAvailable = false;
...
Serial.println(F("WHY DO THE FOLLOWING COMMANDS NOT RETURN OK?"));
    //Send out some initialization to the modem
   // Test for standard modem response
...
{
  for ( int i = 0 ; i < iTimeoutSeconds; i++)
  {
